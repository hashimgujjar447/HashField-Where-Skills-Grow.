...
const CourseContent: React.FC<Props> = ({
  ...
  const toggleSection = (si: number) => {
    const copy = [...sections];
    copy[si] = { ...copy[si], collapsed: !copy[si].collapsed };
    setSections(copy);
  };

  const updateSectionTitle = (si: number, val: string) => {
    const copy = [...sections];
    copy[si] = { ...copy[si], sectionTitle: val };
    setSections(copy);
  };

  const addSection = () => setSections([...sections, defaultSection()]);
  ...
  const updateLecture = (si: number, li: number, field: keyof Lecture, val: any) => {
    const copy = [...sections];
    (copy[si].lectures[li] as any)[field] = val;
    setSections(copy);
  };
  ...
  const updateLink = (
    ...
    const copy = [...sections];
    copy[si].lectures[li].links[lki][field] = val;
    setSections(copy);
  };

  const removeLink = (si: number, li: number, lki: number) => {
    const copy = [...sections];
    copy[si].lectures[li].links = copy[si].lectures[li].links.filter(
      (_, i) => i !== lki
    );
    setSections(copy);
  };
        ...
        {sections.map((section, si) => (
          ...
          >
            <div className="flex items-center gap-2 sm:gap-3 bg-white dark:bg-[#1b2447] px-3 sm:px-4 py-3 border-b border-slate-100 dark:border-slate-700">
              <GripVertical size={17} className="shrink-0 text-slate-300 dark:text-slate-600 cursor-grab" />
            ...
            {!section.collapsed && (
              <div className="p-3 sm:p-4 space-y-3">
                    ...
                    <div className="flex items-center gap-2 sm:gap-2.5 px-3 sm:px-4 py-2.5 border-b border-slate-100 dark:border-slate-700">
                      ...
                      <div className="flex items-center gap-1 ml-1 shrink-0">
                        ...
                        <button
                          ...
                          className="rounded-lg p-1.5 text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-700 transition"
                        >
                          {lecture.collapsed ? <ChevronDown size={16} /> : <ChevronUp size={16} />}
                        </button>
                      </div>
                    </div>
                      ...
                      <div className="space-y-4 p-3 sm:p-4">
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => updateLecture(si, li, "videoLength", e.target.value)}
                              placeholder="15"
                              className={inp}
                              required
                            />
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {lecture.links.map((link, lki) => (
                              <div key={lki} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                                <div className="relative w-full sm:w-36 shrink-0">
                                  <LinkIcon size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                                  <input
                                    value={link.title}
                                    onChange={(e) => updateLink(si, li, lki, "title", e.target.value)}
                                    placeholder="Label"
                                    className={inp + " pl-8"}
                                  />
                                </div>
                                <div className="flex items-center gap-2 flex-1">
                                  <input
                                    value={link.url}
                                    onChange={(e) => updateLink(si, li, lki, "url", e.target.value)}
                                    placeholder="https://..."
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => updateLecture(si, li, "suggestions", e.target.value)}
                ...
                >
                  <Plus size={16} />
                  Add Lecture to &quot;{section.sectionTitle}&quot;
                </button>
              </div>
            )}
          </div>
